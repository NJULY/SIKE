...
from data_process import StructureJointDataModule
from models import NFormer4Joint, Knowformer
...
def get_args():
    ...
    parser.add_argument('--datasets', type=list, default=None)
    parser.add_argument('--alignment_files', type=list, default=None)
...
class NFormerTrainer:
    def __init__(self, config: dict):
        ...
        self.train_dataloader, self.valid_dataloader, self.test_dataloader = self._load_dataset(config)
        self.model_config, self.model = self._load_model(config)

        self.opt, self.scheduler = self.model.configure_optimizers(total_steps=self.epoch)
    ...
    def _load_dataset(self, config: dict):
        data_module = StructureJointDataModule(config)
    ...
    def _load_model(self, config):
        if self.is_validate:
            ...
            model = NFormer4Joint(config, bert_encoder).to(config['device'])
    ...
    def train(self):
        ...
        for i in range(1, self.epoch + 1):
            ...
            # 训练模型
            train_loss = self._train_one_epoch(self.train_dataloader)
            log = f'[train] epoch: {i}, loss: {train_loss}' + '\n'

            # 依次测试每个模型
            test_mrrs = []
            test_scores = dict()
            for dataset in self.valid_dataloader:
                dev_score = self._validate_one_epoch(self.valid_dataloader[dataset])
                test_score = self._validate_one_epoch(self.test_dataloader[dataset])
                test_mrrs.append(test_score['MRR'])
                test_scores[dataset] = test_score

                # 日志
                log += f'{dataset} [valid] epoch: {i}, ' + score2str(dev_score) + '\n'
                log += f'{dataset} [test] epoch: {i}, ' + score2str(test_score) + '\n'

            # 按照测试集大小加权计算MRR
            dataset_size = [len(self.test_dataloader) for dataset in self.test_dataloader]
            dataset_percent = np.array(dataset_size) / np.sum(dataset_size)
            ...
            if best_mrr < test_mrr:
                ...
                save_model(self.model_config, self.model.bert_encoder, model_path)

            # save the best 20 models for SWA
            min_hits1 = min(top_hits1)
            if test_mrr > min_hits1:
                min_idx = top_hits1.index(min_hits1)
                top_hits1[min_idx] = test_mrr
        ...
        log = f'epoch: {best_score["epoch"]} \n'
        for dataset in best_score:
            score = best_score[dataset]
            log += f'{dataset} ' + score2str(score) + '\n'
        print(log)
        with open(self.log_path, 'a') as f:
            f.write(log + '\n')
    ...
    def validate(self):
        ...
        for dataset in self.test_dataloader:
            dev_scores = self._validate_one_epoch(self.valid_dataloader[dataset])
            test_scores = self._validate_one_epoch(self.test_dataloader[dataset])
...
if __name__ == '__main__':
    ...
    # assert 0
    config_from_input = get_args()
    trainer = NFormerTrainer(config_from_input)
    trainer.main()
